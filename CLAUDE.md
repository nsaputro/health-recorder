...
- Keep entries concise — one line per logical change, same level of detail as GitHub release notes
- When a release is cut, the release workflow moves `[Unreleased]` to the version heading; you do not need to do this manually
...
### Fixed
- Weight entry now validates that height_cm is positive before computing BMI
```
...
All supported test types, their display names, default units, and clinical reference ranges live in three dicts: `LAB_TEST_UNITS`, `LAB_TEST_DISPLAY`, `LAB_REFERENCE_RANGES`. The `/health/lab-types` endpoint exposes these to the UI. Unit options per test type (with mg/dL preferred) are defined in `UNIT_OPTIONS` in `ha-addon/ui/index.html`.
...
1. Check the latest release version via `mcp__github__get_latest_release`
2. Bump `version` in `ha-addon/config.yaml` to the next version
3. Move `## [Unreleased]` entries in `CHANGELOG.md` to a new `## [x.y.z] - YYYY-MM-DD` heading and update the comparison links at the bottom
