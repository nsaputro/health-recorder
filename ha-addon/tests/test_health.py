...
class TestLabTypes:
    ...
    def test_gender_ranges_hemoglobin(self, client):
        male   = client.get("health/lab-types?gender=male",   headers=HEADERS_A).json()
        female = client.get("health/lab-types?gender=female", headers=HEADERS_A).json()
...
class TestUserPreferences:
    def test_get_default_unset(self, client):
        r = client.get("auth/preferences", headers=HEADERS_A)
        assert r.status_code == 200
        assert r.json()["gender"] == "unset"

    def test_set_and_get_male(self, client):
        client.put("auth/preferences", json={"gender": "male"}, headers=HEADERS_A)
        r = client.get("auth/preferences", headers=HEADERS_A)
        assert r.json()["gender"] == "male"

    def test_update_gender(self, client):
        client.put("auth/preferences", json={"gender": "male"},   headers=HEADERS_A)
        client.put("auth/preferences", json={"gender": "female"}, headers=HEADERS_A)
        r = client.get("auth/preferences", headers=HEADERS_A)
        assert r.json()["gender"] == "female"

    def test_invalid_gender_rejected(self, client):
        r = client.put("auth/preferences", json={"gender": "other"}, headers=HEADERS_A)
        assert r.status_code == 422

    def test_user_isolation(self, client):
        client.put("auth/preferences", json={"gender": "male"}, headers=HEADERS_A)
        r = client.get("auth/preferences", headers=HEADERS_B)
        assert r.json()["gender"] == "unset"
